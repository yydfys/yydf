...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def getCookie(self,url):
        ...
        if '人机验证' in rsp.text:
            ...
            session.get('https://czzy.fun/a20be899_96a6_40b2_88ba_32f1f75f1552_yanzheng_ip.php?type=96c4e20a0e951f471d32dae103e83881&key={0}&value={1}'.format(key, value), headers=header)
            return session.get(url, headers=header)
        elif '检测中' in rsp.text:
            append = self.regStr(rsp.text, 'href =\"(/.*?)\"')
            session.get('https://czzy.fun{0}'.format(append), headers=header)
            return session.get(url, headers=header)
        else:
            return rsp
